...
# =============================================================================
# Deploy to Production - Lisk Sepolia (Local Simulation)
# =============================================================================
# This script simulates ONLY the "Deploy to Production" section from
# .github/workflows/lisk-sepolia.yml (deploy-lisk-sepolia job)
# =============================================================================
...
# Configuration
DOMAIN="lisk-sepolia-app.scalex.money"
COMPOSE_FILE="docker-compose.lisk-sepolia.yml"

echo -e "${BLUE}=========================================="
echo -e "🚀 DEPLOY TO PRODUCTION - LISK SEPOLIA"
echo -e "==========================================${NC}"
echo ""
...
deploy_to_production() {
    log_info "Deploying Lisk Sepolia frontend..."
    echo ""
    ...
    cp apps/web/.env.lisk-sepolia apps/web/.env
...
show_container_logs() {
    ...
    docker-compose -f "$COMPOSE_FILE" logs --tail=100 frontend-lisk-sepolia
    echo ""

    echo "=== 3. STARTUP VERIFICATION ==="
    # Check if container is running
    CONTAINER_STATUS=$(docker-compose -f "$COMPOSE_FILE" ps -q frontend-lisk-sepolia)
    if [ -n "$CONTAINER_STATUS" ]; then
        ...
        if timeout 30s docker-compose -f "$COMPOSE_FILE" exec -T frontend-lisk-sepolia curl -f http://localhost/health > /dev/null 2>&1; then
...
run_health_check() {
    ...
    LOCAL_PORT=$(docker-compose -f "$COMPOSE_FILE" port frontend-lisk-sepolia 80 2>/dev/null | cut -d':' -f2)
    ...
    if [ -n "$LOCAL_PORT" ]; then
        ...
        for attempt in {1..5}; do
            ...
            if [ "$response" = "200" ]; then
                ...
                if [ $attempt -eq 5 ]; then
                    ...
                    docker-compose -f "$COMPOSE_FILE" logs --tail=20 frontend-lisk-sepolia
                    echo "Debugging - checking container status:"
                    docker-compose -f "$COMPOSE_FILE" ps
                    echo "Debugging - testing local connectivity:"
                    docker-compose -f "$COMPOSE_FILE" exec -T frontend-lisk-sepolia curl -f http://localhost/health || echo "Local health check failed"
                    exit 1
                fi
            fi
        done
        ...
        if docker-compose -f "$COMPOSE_FILE" exec -T frontend-lisk-sepolia curl -f http://localhost/health > /dev/null 2>&1; then
...
verify_container_status() {
    ...
    FRONTEND_STATUS=$(docker-compose -f "$COMPOSE_FILE" ps -q frontend-lisk-sepolia)
    if [ -n "$FRONTEND_STATUS" ]; then
        ...
        if docker-compose -f "$COMPOSE_FILE" exec -T frontend-lisk-sepolia curl -f http://localhost/health > /dev/null 2>&1; then
            ...
            docker-compose -f "$COMPOSE_FILE" logs --tail=15 frontend-lisk-sepolia
            exit 1
        fi
        ...
        docker-compose -f "$COMPOSE_FILE" logs frontend-lisk-sepolia
        docker-compose -f "$COMPOSE_FILE" ps -a
        exit 1
    fi
    echo ""
