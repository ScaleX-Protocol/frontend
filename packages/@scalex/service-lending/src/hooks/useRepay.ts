...
export function useRepay({ onSuccess, onError }: UseRepayOptions = {}) {
  ...
  const getRouterAddress = useCallback((chainId?: number) => {
    ...
    if (!chainContracts) {
      ...
      console.error('ScaleXRouter contract not found', { targetChainId, availableChains }, 'getRouterAddress', 'useRepay.ts');
      throw error;
    }
  ...
  const executeTransaction = useCallback(async (contractCall: any) => {
    ...
    try {
      ...
      try {
        ...
        console.error('Transaction simulation failed', { error: simulationError.message || simulationError }, 'executeTransaction', 'useRepay.ts');
      ...
      if (txReceipt.status === 'reverted') {
        logger.log(LogLevel.ERROR, 'Transaction failed on-chain',);
