...
import { VictoryLine, VictoryChart, VictoryAxis, VictoryArea, VictoryCandlestick } from 'victory-native';
import {
  useMarkets,
  useTicker24hr,
  useKline,
...
function TradeScreenContent() {
  ...
  const intervals: Array<'1m' | '5m' | '30m' | '1h' | '1d'> = ['1m', '5m', '30m', '1h', '1d'];

  // Fetch markets
  const { data: markets, isLoading: marketsLoading, isFetching: marketsFetching, refetch: refetchMarkets } = useMarkets();
  ...
  const { data: ticker, isFetching: tickerFetching, refetch: refetchTicker } = useTicker24hr(
    selectedMarket || '',
    { enabled: !!selectedMarket, refetchInterval: 5000 }
  );

  // Fetch kline data for chart
  const { data: klineData, isFetching: klineFetching, refetch: refetchKline } = useKline(
    { symbol: selectedMarket || '', interval, limit: 100 },
    { enabled: !!selectedMarket }
  );

  // Fetch order book with real-time updates
  const { data: orderBook, isLoading: orderBookLoading, isFetching: orderBookFetching } = useDepthWithRealtime({
  ...
  const { data: trades, isLoading: tradesLoading, isFetching: tradesFetching } = useTradesWithRealtime({
    symbol: selectedMarket || '',
    limit: 50,
    enableRealtime: true,
  });
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
          ...
          <>
            <View style={styles.priceSection}>
              ...
              <Text style={[
                ...
                <Text style={styles.priceChangeDetail}>
                  {priceChange.isPositive ? '+' : ''}${priceChange.value.toFixed(2)} (24h)
                </Text>
              </Text>
            </View>
            ...
            <View style={styles.statsRow}>
              ...
              <View style={styles.statItem}>
                ...
                <Text style={styles.statValue}>
                  ${(parseFloat(ticker?.quoteVolume || '0') / 1000000).toFixed(2)}M
                </Text>
              </View>
            </View>
          </>
        ...
        <View style={styles.timePeriodContainer}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            {intervals.map((int) => (
              <TouchableOpacity
                key={int}
                style={[styles.timePeriodButton, interval === int && styles.timePeriodActive]}
                onPress={() => setInterval(int)}
              >
                <Text style={[styles.timePeriodText, interval === int && styles.timePeriodTextActive]}>
                  {int}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        <View style={styles.buySellToggle}>
          <TouchableOpacity
            style={[styles.toggleButton, activeOrderType === 'buy' && styles.toggleButtonBuyActive]}
            onPress={() => setActiveOrderType('buy')}
          >
            <Text style={[styles.toggleText, activeOrderType === 'buy' && styles.toggleTextActive]}>
              Buy
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.toggleButton, activeOrderType === 'sell' && styles.toggleButtonSellActive]}
            onPress={() => setActiveOrderType('sell')}
          >
            <Text style={[styles.toggleText, activeOrderType === 'sell' && styles.toggleTextActive]}>
              Sell
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.orderModeTabs}>
          <TouchableOpacity
            style={[styles.orderModeTab, orderMode === 'limit' && styles.orderModeTabActive]}
            onPress={() => setOrderMode('limit')}
          >
            <Text style={[styles.orderModeText, orderMode === 'limit' && styles.orderModeTextActive]}>
              Limit
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.orderModeTab, orderMode === 'market' && styles.orderModeTabActive]}
            onPress={() => setOrderMode('market')}
          >
            <Text style={[styles.orderModeText, orderMode === 'market' && styles.orderModeTextActive]}>
              Market
            </Text>
          </TouchableOpacity>
        </View>
              ...
              <View>
                {openOrders.map((order, index) => (
                  <View key={`order-${index}`} style={styles.orderRow}>
                    <Text style={[
                      styles.orderSide,
                      order.side === 'BUY' ? { color: '#2ECC71' } : { color: '#E74C3C' }
                    ]}>
                      {order.side}
                    </Text>
