...
export function PortfolioAssetsTable({ data }: PortfolioAssetsTableProps) {
  return (
    <View>
      {/* Table Header */}
      ...
      {data.map((asset, index) => (
        <View key={asset.id || index} style={styles.row}>
          ...
          <View style={[styles.cell, { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 8 }]}>
            <TokenIcon symbol={`sx${asset.asset}`} size="sm" />
            <Text style={styles.assetText}>sx{asset.asset}</Text>
          </View>
