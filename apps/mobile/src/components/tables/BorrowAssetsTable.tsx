...
interface BorrowAssetsTableProps {
  data: LendingBorrow[];
}

export function BorrowAssetsTable({ data }: BorrowAssetsTableProps) {
  return (
    <View>
      {/* Table Header */}
      <View style={styles.headerRow}>
        <Text style={[styles.headerText, { flex: 1 }]}>ASSET</Text>
        <Text style={[styles.headerText, { flex: 1, textAlign: 'center' }]}>DEBT</Text>
        ...
        <View key={asset.id || index} style={styles.row}>
          ...
          <View style={[styles.cell, { flex: 1, alignItems: 'center' }]}>
            <Text style={styles.valueText}>
              {parseFloat(asset.borrowedAmount).toFixed(4)} {asset.asset}
            </Text>
          </View>
