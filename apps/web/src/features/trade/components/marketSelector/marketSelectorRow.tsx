...
export function MarketSelectorRow({
  ...
  const { data: ticker } = useTicker24h(symbol);
...
// Mobile Card variant for market selector
interface MarketSelectorCardProps {
  market: Market;
  isSelected: boolean;
  isFavorite: boolean;
  onSelect: (market: Market) => void;
...
export function MarketSelectorCard({
  market,
  isSelected,
  isFavorite,
  onSelect,
  ...
  return (
    <div 
      className={`p-3 rounded-lg cursor-pointer transition-colors ${
        isSelected 
          ? 'bg-[#2A2A2A] border border-[#F06718]' 
          : 'bg-[#2A2A2A]/50 border border-transparent hover:bg-[#2A2A2A]'
      }`}
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <span className="text-[#E0E0E0] font-medium text-sm">{market.baseAsset}/{market.quoteAsset}</span>
              <span className="px-1.5 py-0.5 text-[8px] font-medium bg-[#3A3A3A] text-[#A0A0A0] rounded">
                SPOT
              </span>
            </div>
            <span className="text-[10px] text-[#A0A0A0]">Vol: {displayData.volume}</span>
          </div>
        </div>

        {/* Right: Price info */}
        <div className="flex flex-col items-end">
          <span className="text-[#E0E0E0] font-mono text-sm">${displayData.lastPrice}</span>
          <span className={`text-xs font-mono ${displayData.isPositive ? 'text-green-400' : 'text-red-400'}`}>
            {displayData.isPositive ? '+' : ''}{displayData.priceChangePercent}%
          </span>
        </div>
      </div>
    </div>
