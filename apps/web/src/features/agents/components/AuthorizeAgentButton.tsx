...
type TxStep = 'idle' | 'selecting' | 'confirming' | 'pending' | 'syncing' | 'completed' | 'error';
...
export default function AuthorizeAgentButton({ agentTokenId, walletAddress }: AuthorizeAgentButtonProps) {
  ...
  const isAuthorized = policyData?.data && !Array.isArray(policyData.data) && (policyData.data as AgentInstallation).enabled;

  const handleAuthorize = useCallback(async (policy: PolicyStruct) => {
    setStep('confirming');
    setError(null);
  ...
  const handleRevoke = useCallback(async () => {
    if (!confirm('Are you sure you want to revoke this agent? It will no longer be able to trade on your behalf.')) return;
  ...
  const isProcessing = step === 'confirming' || step === 'pending' || step === 'syncing';
    ...
    <div className="space-y-2">
      ...
      {step === 'completed' && (
        <p className="text-center text-sm text-green-400">
          {isAuthorized ? 'Agent revoked successfully' : 'Agent authorized successfully'}
        </p>
      )}
