...
interface PolicyTemplateSelectorProps {
  ...
  onCustomize: (templateIndex?: number) => void;
  onCancel: () => void;
  isLoading?: boolean;
}
          ...
          {POLICY_TEMPLATES.map((template, index) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {selectedIndex === index && (
                  <Check size={16} className="text-[#F06718]" />
                )}
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Settings size={14} className="text-[#F06718]" />
                <h3 className="font-semibold text-sm text-[#FFFFFF]">
                  Custom Policy
                </h3>
              </div>
            ...
            <p className="text-xs text-[#808080] mt-1">
              Configure all policy parameters manually
            </p>
          </button>
        </div>
          ...
          <button
            ...
            onClick={() => {
              if (selectedIndex === "custom") {
                ...
                onSelect(POLICY_TEMPLATES[selectedIndex as number].policy);
              }
            }}
            disabled={isLoading}
